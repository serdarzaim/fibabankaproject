...
public class TransactionRepositoryImpl implements TransactionRepository {
    ...
    private static final String SQL_FIND_ALL = "SELECT TRANSACTION_ID, SNDR_ACCOUNT_ID, SNDR_USER_ID, RCPT_ACCOUNT_ID,  RCPT_USER_ID, AMOUNT, NOTE, TRANSACTION_DATE FROM BA_TRANSACTIONS WHERE SNDR_ACCOUNT_ID = ? AND SNDR_ACCOUNT_ID = ?";
    private static final String SQL_FIND_BY_ID = "SELECT TRANSACTION_ID, ACCOUNT_ID, USER_ID, AMOUNT, NOTE, TRANSACTION_DATE FROM BA_TRANSACTIONS WHERE USER_ID = ? AND ACCOUNT_ID = ? AND TRANSACTION_ID = ?";
    private static final String SQL_CREATE = "INSERT INTO BA_TRANSACTIONS "
    		+ "(TRANSACTION_ID, SNDR_ACCOUNT_ID, SNDR_USER_ID, RCPT_ACCOUNT_ID,  RCPT_USER_ID, AMOUNT, NOTE, TRANSACTION_DATE) "
    		+ "VALUES(NEXTVAL('BA_TRANSACTIONS_SEQ'), ?, ?, ?, ?, ?, ?, ?)";
    private static final String SQL_UPDATE = "UPDATE BA_TRANSACTIONS SET AMOUNT = ?, NOTE = ?, TRANSACTION_DATE = ? WHERE USER_ID = ? AND ACCOUNT_ID = ? AND TRANSACTION_ID = ?";
    private static final String SQL_DELETE = "DELETE FROM BA_TRANSACTIONS WHERE USER_ID = ? AND ACCOUNT_ID = ? AND TRANSACTION_ID = ?";
    ...
    @Override
    public Integer create(Integer senderAccountId, Integer senderUserId, Integer recipientAccountId, Integer recipientUserId, Double amount, String note, Long transactionDate) throws EtBadRequestException {
        try {
            ...
            jdbcTemplate.update(connection -> {
                ...
                ps.setInt(3, recipientAccountId);
                ps.setInt(4, recipientUserId);
                ps.setDouble(5, amount);
                ps.setString(6, note);
                ps.setLong(7, transactionDate);
                return ps;
            }, keyHolder);
    ...
    public Boolean transferMoney(Integer senderAccountID, Integer receiverAccountID, Double amount, String note) throws EtBadRequestException {
        try {
        	Account senderAccount = (Account) jdbcTemplate.queryForObject(SQL_GET_ACCOUNT_BY_ID, new Object[]{senderAccountID}, accountRowMapper);
        	Account receiverAccount = (Account) jdbcTemplate.queryForObject(SQL_GET_ACCOUNT_BY_ID, new Object[]{receiverAccountID}, accountRowMapper);
        	
            if(senderAccount.getUsrId() == receiverAccount.getUsrId()) {
            	
            	if(senderAccount.getCurrency().equals(receiverAccount.getCurrency())) {
            		
            		create(senderAccount.getAccountId(),senderAccount.getUsrId(),receiverAccount.getAccountId(),receiverAccount.getUsrId(),amount,"Hesaplarım arası giden para", new Date().getTime());
            		
            		Double newSenderBalance = senderAccount.getBalance()-amount;
            		Double newReceiverBalance = receiverAccount.getBalance()+amount;
            		jdbcTemplate.update(SQL_UPDATE_ACCOUNT_BALANCE, new Object[]{newSenderBalance, senderAccountID});
            		jdbcTemplate.update(SQL_UPDATE_ACCOUNT_BALANCE, new Object[]{newReceiverBalance, receiverAccountID});
