...
public class UserRepositoryImpl implements UserRepository{
    ...
    private static final String SQL_UPDATE_BY_TCKN = "UPDATE BA_USERS SET FIRST_NAME = ?, LAST_NAME = ?, EMAIL = ? WHERE TCKN = ? ";
	...
	public User[] getAllUsers() {
		return jdbcTemplate.queryForObject(SQL_FIND_ALL_USERS, User[].class);
	}
	...
	public Boolean updateUser(User user) {
        try {
            ...
            if(BCrypt.checkpw(user.getPassword(), checkUser.getPassword())) {
            	jdbcTemplate.update(SQL_UPDATE_BY_TCKN, new Object[]{user.getFirstName(), user.getLastName(), user.getEmail(), user.getTckn()});
            	return true;
            }else {
                return false;
            }
